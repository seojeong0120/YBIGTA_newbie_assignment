...
class MultiLayerPerceptron(object):
    ...
    def forward_propagation(self, X):
        ...
        # Layer 4 (hidden layer 3 to output layer)
        h4 = h3 @ W4 + b4
    ...
    def train(self, X_train, y_train, X_val=None, y_val=None, learning_rate=1e-3, L2_norm=0.0, epoch=20000, print_loss=True):
        ...
        for it in range(epoch):
            ...
            # Forward propagation
            # y_hat: predicted output, cache: values needed for backpropagation
            y_hat, cache = self.forward_propagation(X_train)
            # Compute loss
            # y_train: true labels, L2_norm: regularization strength
            loss = self.compute_loss(y_hat, y_train, L2_norm)
            # Back propagation
            # cache: values needed for backpropagation, X_train: training data, y_train: true labels, L2_norm: regularization strength
    ...
    def predict(self, X):
        ...
        predictions = (y_hat >= 0.5).astype(int)  # Convert probabilities to binary predictions
...
def relu(x):
    ### CODE HERE ###
    x = np.maximum(0, x)  # Apply ReLU activation function
    ############################
    return x
...
def leakyrelu(x):
    ### CODE HERE ###
    x = np.where(x > 0, x, 0.01 * x)  # Apply Leaky ReLU activation function
    ############################
    return x 
